...
create user 'jh'@'%' identified by 'jh980101^^';
```mysql
...
## 발생한 문제
### 테스트환경과 분리되지 않음
테스트환경에서 사용하던 메모리 DB 데이터소스와 새롭게 설정한 
```java
@Configuration  
@Slf4j  
public class DatasourceConfig {  
...
##### 프로필 미적용
3. 모듈의 테스트 `application.yml`에 `test` 프로필을 적용하지 않음
4. `LazyConnectionDataSourceProxy`를 사용하려 함
5. master db, slave db의 환경변수를 테스트에서는 적용하지 않음
6. 데이터베이스에 연결하지 못해 에러 발생
##### 테스트에서 메모리 DB 데이터소스가 적용되지 않음
`@Profile("test")` 애노테이션을 사용했기 때문에 테스트 프로필에서는 testDataSource()에서 반환되는 h2 메모리 DB 데이터소스를 사용할 것으로 기대하였으나 `dataSource()` 메서드에 @Primary 애노테이션이 적용되어 있었기 때문에 우선적으로 빈으로 등록되었다. dataSource() 메서드에서 사용하는 데이터소스 정보는 설정파일에 환경변수로 주어지는데, 테스트 환경에서는 환경변수를 제공하지 않았기 때문에 데이터소스의 정보를 파악하지 못해 에러가 발생한 것이다.

이 문제를 해결하기 위해서는 테스트 환경에서 다시 h2 메모리 DB 데이터소스를 사용하도록 수정해야한다. 프로필이 test일 때 사용할 데이터소스 에도 `@Primary` 애노테이션을 붙여주고, 프로필이 test가 아닌 환경에서 사용할 설정에는 test 프로필이 아닐 때 사용한다는 `@Profile("!test")` 프로필을 사용하면 문제를 해결할 수 있다.
- 테스트 환경에서 사용할 데이터소스 빈을 반환하는 메서드에도 `@Primary`를 적용한다.
- 운영환경에서 사용할 데이터소스 빈을 반환하는 메서드에도 `@Primary`가 붙어있다. 따라서 test 프로필이 아닐 때 사용한다는 표시를 꼭 해주어야한다. `@Profile("!test")`
